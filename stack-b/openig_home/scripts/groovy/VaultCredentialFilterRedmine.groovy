...
try {
    ...
    def email = attributes.openid['user_info']['email'] as String
    ...
    def redmineLogin = credsJson?.data?.data?.login as String
    def redminePassword = credsJson?.data?.data?.password as String
    if (redmineLogin == null || redmineLogin.isEmpty() || redminePassword == null || redminePassword.isEmpty()) {
        throw new IllegalStateException('Vault response is missing login or password')
    }

    attributes.redmine_credentials = [login: redmineLogin, password: redminePassword]
