...
|--------|---------|
| Implementation status | `IMPLEMENTED` |
| Code evidence | Redis write path in `stack-a/openig_home/scripts/groovy/BackchannelLogoutHandler.groovy:315-334`, `stack-b/openig_home/scripts/groovy/BackchannelLogoutHandler.groovy:335-354`, `stack-c/openig_home/scripts/groovy/BackchannelLogoutHandler.groovy:335-354`; current TTL `EX 1800` in the consolidated handler, aligned to `JwtSession.sessionTimeout: "30 minutes"` (current-state commit 9cbf71a) |
| Architecture assessment | `ADEQUATE` |
| Security risk | Revocation storage exists and the consolidated handler now writes blacklist entries with TTL `1800`, aligned to `JwtSession.sessionTimeout: "30 minutes"`. Redis AOF persistence (`appendonly yes`, `appendfsync everysec`) is also enabled across all 3 stacks. Remaining concern is operational drift: if session timeout changes again, blacklist TTL must be kept in sync. |
| Standards reference | OpenID Connect Back-Channel Logout 1.0; RFC 7009 (revocation reliability expectations) |
| Recommended action | Keep blacklist TTL and `JwtSession.sessionTimeout` aligned whenever either value changes; Redis persistence is already enabled, so no additional durability workaround is currently required. |
...
| ~~Blacklist durability is not evidenced and TTL is fixed at `3600`~~ | ~~`H9`~~ | ~~`MEDIUM`~~ | ~~Redis restart can erase revocations, and active sessions can outlive blacklist entries~~ | **RESOLVED** — consolidated `BackchannelLogoutHandler` now writes `EX 1800`, matching `JwtSession.sessionTimeout: "30 minutes"` (commit 9cbf71a), and Redis AOF persistence is enabled across all 3 stacks |
...
|---------|------------------|------|--------------------|
| OpenIG `JwtSession` | `stack-a/openig_home/config/config.json:20-29`, `stack-b/openig_home/config/config.json:11-18`, `stack-c/openig_home/config/config.json:20-29` show `sessionTimeout: "30 minutes" (updated 2026-03-16, was 8 hours)` | Gateway session can outlive blacklist TTL and some upstream app sessions | Align `JwtSession` timeout with Keycloak realm session lifetime and use the same expiry basis for revocation TTL |
| Redis blacklist entry | Consolidated `BackchannelLogoutHandler` writes `EX 1800`, aligned to `JwtSession.sessionTimeout: "30 minutes"` (commit 9cbf71a) | Gateway/session TTL mismatch is resolved; future drift would reintroduce the problem | Keep blacklist TTL and gateway session timeout coupled whenever one changes |
...
- Browser closes without hitting an app logout intercept: OpenIG keeps a `30 minutes` session and may retain upstream app state in that same session (`stack-a/openig_home/config/config.json:24`, `stack-c/openig_home/config/config.json:24`, `stack-a/openig_home/scripts/groovy/CredentialInjector.groovy:94-96`).
- Redis restarts after a blacklist write: AOF persistence now preserves the revocation marker across restart, but the marker still expires when the aligned 30-minute gateway session window expires (commit 9cbf71a).
- Redis is down during blacklist check: the filter now fails closed with `500`, which protects downstream apps but temporarily blocks authenticated traffic until Redis recovers (`SessionBlacklistFilter` FIX-03, commit 278a29c).
...
Interpretation: the gateway implements the full SSO/SLO chain and backchannel logout JWT validation is now fully implemented across all three stacks (H8 resolved). The remaining open items are hardening tasks rather than functional holes, led by plaintext Vault transport (`H4`) and HTTP-only OIDC endpoints (`H3`). The earlier blacklist fail-open (`H10`) and TTL/durability mismatch (`H9`) findings are now resolved.
