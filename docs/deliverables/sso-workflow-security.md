...
| Architecture assessment | `ADEQUATE` |
| Security risk | Revocation storage exists, but ~~TTL is fixed at one hour instead of the remaining user session lifetime~~ **UPDATE (2026-03-16):** TTL now aligned to 1800s (30 min), matching JwtSession timeout. ~~No Redis persistence setting is evidenced in the reviewed repo, so restart durability of blacklist entries is uncertain.~~ **UPDATE (2026-03-16):** Redis AOF persistence enabled (appendonly yes, appendfsync everysec) across all 3 stacks — blacklist keys verified to survive restart. |
