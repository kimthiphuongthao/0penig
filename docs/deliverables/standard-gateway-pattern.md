...
> **Lab Exception (FIX-07 Phase 7a / C-2-S-1):** All traffic in this lab runs over plaintext HTTP. `requireHttps: false` is intentional — setting it to `true` without TLS infrastructure in place causes every OIDC flow to fail immediately. That means the browser-carried `JwtSession` cookie (`IG_SSO*`) is also a lab-only HTTP exception until nginx TLS termination is live. Full HTTPS requires: a self-signed CA distributed to all containers, nginx TLS termination configured per stack, Docker networking changes to route port 443, and the CA cert trusted by the JVM inside OpenIG. Phase 7b (actual TLS with self-signed CA and nginx termination) is deferred to the Vault Production Hardening phase. Before any production deployment, complete that TLS termination step, then switch `JwtSession` cookies to HTTPS-only transport with the `Secure` flag and set every `OAuth2ClientFilter` to `requireHttps: true`. Until Phase 7b is complete, this lab is not a production reference for transport security.
