...
|---|---------|--------|-------|--------|
| C-1 | JWKS cache race condition — non-atomic check-then-act on 2 volatile fields | Code Review | 3x BackchannelLogoutHandler.groovy | RESOLVED 2026-03-17 by commit `4d8f065` (`globals.compute` JWKS cache) |
...
| # | Finding | Source | Files | Effort |
|---|---------|--------|-------|--------|
| H-1 | SloHandler missing try-catch (3 files) | Code+Security+Architecture | SloHandler.groovy (A), SloHandlerGrafana.groovy (C), SloHandlerPhpMyAdmin.groovy (C) | RESOLVED 2026-03-17 by commit `3b8a6d8` (SloHandler try-catch) |
...
| H-9 | Stack C nginx missing proxy_buffer_size 128k | Architecture | stack-c/nginx/nginx.conf | RESOLVED 2026-03-17 by commit `f86c7eb` (Stack C nginx proxy buffers) |
...
| M-10 | Stack A SloHandler hardcoded Keycloak URL (no env var) | Code+Architecture | RESOLVED 2026-03-17 by commit `3b8a6d8` (`KEYCLOAK_BROWSER_URL`) |
...
| M-14 | App1ResponseRewriter.groovy dead code (0 bytes) | Code+Analyst | RESOLVED 2026-03-17 by commit `f86c7eb` (`App1ResponseRewriter.groovy` deleted) |

---

## Priority 4 — Consolidation (historical plan; resolved)

| # | Task | Lines Saved | Status / prerequisite |
|---|------|-------------|---------------------|
| P-1 | Parameterize BackchannelLogoutHandler via `args` (3 → 1 file) | ~693 | **RESOLVED** by commit `4d8f065` |
| P-2 | Parameterize SessionBlacklistFilter (6 → 1 file, Stack C pattern) | ~597 | **RESOLVED** by commits `a76e194` + `832bbae` |
| P-3 | Extract shared Vault login utility | ~196 | Still open — verify OpenIG Groovy classpath / `evaluate()` support |
| P-4 | Parameterize SloHandler via `args` (5 → 1-2 files) | ~190 | **RESOLVED** by commit `3b8a6d8` |
| **Total** | | **~1676** | |

Pattern consolidation summary: RESOLVED by commits `a76e194` + `832bbae` (SessionBlacklistFilter), `4d8f065` (BackchannelLogoutHandler), `3b8a6d8` (SloHandler).
...
## Quick Win Batch (completed in Step 5)

Completed items:
- H-2: Add `vault/keys/` to `.gitignore`
- H-3: Remove Redmine port 3000 exposure
- H-9: Add proxy_buffer_size / proxy_buffers to Stack C nginx
...
**Outcome:** Complete. Validation PASS: 5 backchannel clients blacklisted, 5 logout redirects correct, phpMyAdmin inline failureHandler OK.
