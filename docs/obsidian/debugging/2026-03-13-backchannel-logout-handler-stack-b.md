---
title: Stack B BackchannelLogoutHandler fixes
tags:
  - debugging
  - openig
  - keycloak
...
# Stack B BackchannelLogoutHandler fixes

Context: fixed four scoped bugs in [[OpenIG]] backchannel logout handling for [[Stack B]] at [stack-b/openig_home/scripts/groovy/BackchannelLogoutHandler.groovy](/Volumes/OS/claude/openig/sso-lab/stack-b/openig_home/scripts/groovy/BackchannelLogoutHandler.groovy).
...
> `EXPECTED_AUDIENCE` remains `openig-client-b`.
> `REDIS_HOST` default remains `redis-b`.
> The request flow and Redis blacklist write path were not changed.
...
- Stack B backchannel logout script now targets the correct realm.
