...
Lý do: OpenIG lưu token theo key gắn với public URL đầy đủ (bao gồm scheme + host + port + clientEndpoint). Sai key => script không đọc được `id_token` => logout/blacklist SID hỏng.
