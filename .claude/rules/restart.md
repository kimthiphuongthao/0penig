...
docker exec -e VAULT_ADDR=http://127.0.0.1:8200 -e VAULT_TOKEN="$ADMIN_TOKEN" sso-vault \
...
ADMIN_TOKEN_B=$(cat vault/keys/.vault-keys.admin)
docker exec -e VAULT_ADDR=http://127.0.0.1:8200 -e VAULT_TOKEN="$ADMIN_TOKEN_B" sso-b-vault \
...
ADMIN_TOKEN_C=$(cat vault/keys/.vault-keys.admin)
docker exec -e VAULT_ADDR=http://127.0.0.1:8200 -e VAULT_TOKEN="$ADMIN_TOKEN_C" stack-c-vault-c-1 \
