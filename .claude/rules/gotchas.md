...
| Vault Transit engine not auto-applied to existing Vault instance | vault-bootstrap.sh changes only take effect when re-run against the live Vault container. If Vault was already running before enable_transit() was added to bootstrap, the Transit engine and per-app keys are NOT present. Symptom: HTTP 403 on transit/encrypt + JWT session is too large (oauth2 blob cannot offload to Redis, cookie overflow). Fix: re-run bootstrap, update policies via root token (requires vault operator generate-root), regenerate AppRole secret_ids | Always re-run vault-bootstrap.sh after adding new Vault config; verify with vault secrets list and vault list transit/keys after any bootstrap change |
