...
| Backchannel logout lost khi Redis down | BackchannelLogoutHandler trả 500 (FIX-05) nhưng blacklist key không được ghi. Khi Redis recovery, session vẫn valid (chưa bị revoke) cho đến khi JwtSession hết hạn tự nhiên (8h) | Production: Redis HA (Sentinel/Cluster) + Keycloak retry config. Lab: ~~accepted limitation~~ **RESOLVED** — appendonly yes enabled (commit 00f5558), blacklist keys survive restart |
