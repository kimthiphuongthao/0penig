...
| Stack B entrypoint `cp -r` không overwrite | `docker-entrypoint.sh` dùng `cp -r /opt/openig /tmp/openig` — khi `/tmp/openig` đã tồn tại (restart), cp tạo subdirectory thay vì overwrite → file cũ vẫn được dùng | Sau khi sửa file, phải `docker cp` trực tiếp vào container hoặc `docker compose down && up` (full recreate) |
